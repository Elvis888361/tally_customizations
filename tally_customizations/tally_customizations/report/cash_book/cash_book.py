...
def get_data(filters):
	...
	for gle in gl_entries:
		...
		particulars = format_cash_book_particulars(gle)
...
def get_gl_entries(filters, account_list):
	"""Fetch GL entries for cash accounts for the selected period"""
	if not account_list:
		return []
...
def format_cash_book_particulars(gle):
	...
	if gle.get("debit") > 0:
		...
		prefix = "Cr"
		contra_account = gle.get("against") or gle.get("party") or "Sales"
	elif gle.get("credit") > 0:
		# Credit entry - money going out (Debit to the contra account)
		prefix = "Dr"
		contra_account = gle.get("against") or gle.get("party") or "Expenses"
